#pragma once
	...
	#define RBC_BIG_ENDIAN __STDC_ENDIAN_BIG__
	#define RBC_LITTLE_ENDIAN __STDC_ENDIAN_LITTLE__
	#define RBC_BYTE_ORDER __STDC_ENDIAN_NATIVE__
#else
    ...
    // Machine byte-order, reuse preprocessor provided macros when available
		...
		#if defined(__LITTLE_ENDIAN__) || (defined(__BYTE_ORDER__) && __BYTE_ORDER__ == RBC_LITTLE_ENDIAN)
			#define RBC_BYTE_ORDER RBC_LITTLE_ENDIAN
		#else
			#define RBC_BYTE_ORDER RBC_BIG_ENDIAN
		#endif
