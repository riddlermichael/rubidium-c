...
RbcError rbc_tls_init(rbc_tls* self, rbc_tls_dtor_t dtor) {
	RBC_SYNC_INIT(rbc_tls);
	RBC_SYNC_CHECK_INIT(pthread_key_create(RBC_SYNC_IMPL_PTR, dtor));
}

RbcError rbc_tls_destroy(rbc_tls* self) {
	RBC_SYNC_DESTROY(pthread_key_delete(*RBC_SYNC_IMPL_PTR));
}

RbcError rbc_tls_get(rbc_tls self, void** value) {
...
RbcError rbc_tls_set(rbc_tls self, void const* value) {
