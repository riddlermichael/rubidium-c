...
#define RBC_FROM_CHARS_IMPL_SIGNED(type)                                      \
	RbcFromCharsResult RBC_CONCAT(rbc_from_chars_, type)(                     \
	    char const* first,                                                    \
	    char const* last,                                                     \
	    type* value) {                                                        \
		type const max = RBC_CONCAT(type, max);                               \
		type const min = RBC_CONCAT(type, min);                               \
		RbcFromCharsResult result;                                            \
		result.ptr = first;                                                   \
...
#define RBC_TO_CHARS_IMPL_SIGNED(type)                                      \
	RbcToCharsResult RBC_CONCAT(rbc_to_chars_, type)(                       \
	    char* first,                                                        \
	    char* last,                                                         \
	    type value) {                                                       \
		if (last <= first) {                                                \
			return (RbcToCharsResult) {first, RBC_ERROR_INVALID_ARGUMENT};  \
		}                                                                   \
                                                                            \
		if (value == 0) {                                                   \
			*first++ = '0';                                                 \
			return (RbcToCharsResult) {first, RBC_ERROR_OK};                \
		}                                                                   \
                                                                            \
		RbcToCharsResult result;                                            \
		result.ptr = last;                                                  \
